...
# VIRTUALENV
@given(u'que temos o VIRTUALENV instalado')
def step_impl(context):
    pass


@when(u'eu executo o comando "{command}" no terminal para verificar a versao do VIRTUALENV')
def step_impl(context, command):
    context.completed_subprocess = subprocess.Popen([command], shell=True, stdout=subprocess.PIPE,
                                                    stderr=subprocess.STDOUT, close_fds=True)


@then(u'eu deveria ver a versao do VIRTUALENV')
def step_impl(context):
